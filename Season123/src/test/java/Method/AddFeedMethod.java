...
public  void btnAddPost() throws Exception, InterruptedException {
	Utils.waitUntil(feed.Feed_plus_icon);
	feed.Feed_plus_icon.click();
	Utils.waitUntil(feed.Create_a_post);
	feed.Create_a_post.click();
}
...
public  void btnPost1() throws Exception {
	System.out.println("waiting");
	Utils.waitUntil(feed.btnPost1);
...
public void tagLocation1() throws Exception{
	Utils.waitUntil(feed.tagLocation1);
	feed.tagLocation1.click();
}
...
public  void editImageFeed() throws Exception {
	Utils.waitUntil(feed.ellipsis4);
	 feed.ellipsis4.click();	
	 Utils.waitUntil(feed.btnEdit);
	 feed.btnEdit.click();
	 try{
		 if(feed.removeImage != null && feed.removeImage.isDisplayed()) {
...
public  void Add_a_photo_by_camera() throws Exception, InterruptedException {
	 Utils.waitUntil(feed.add_a_photo);
	 feed.add_a_photo.click();
	 Utils.waitUntil(feed.TakePhoto);
	 feed.TakePhoto.click();
	 Utils.waitUntil(feed.Click_To_Take_Photo);
	 feed.Click_To_Take_Photo.click();
	 Thread.sleep(2000);
	 Utils.waitUntil(feed.btnOk);
	 feed.btnOk.click();
//	if(feed.add_a_photo != null && feed.add_a_photo.isDisplayed()) {
//		feed.add_a_photo.click(); 
//	}
//	else {
//		Utils.waitUntil(feed.addPhoto);
...
public  void Add_a_photo_by_Library() throws Exception, InterruptedException {
	 Utils.waitUntil(feed.add_a_photo);
	 feed.add_a_photo.click();
	 Utils.waitUntil(feed.ChooseFromLibrary);
	 feed.ChooseFromLibrary.click();
	 Utils.waitUntil(feed.OpenAlbum);
	 feed.OpenAlbum.click();
	 Utils.waitUntil(feed.SelectImage);
	 feed.SelectImage.click();
...
public  void addImageByCamera() throws Exception, InterruptedException {
	...
	Utils.waitUntil(feed.commentAddPhoto);
	 ...
	 //feed.btnImageOpen.click();
	 Utils.waitUntil(feed.TakePhoto);
	 feed.TakePhoto.click();
	 //Below code is not needed (Photo Permision)
//	 try{
//		 if(feed.locationPopover != null && feed.locationPopover.isDisplayed()) {
